...
class NovAdvancedSearch extends Module
{
	...
	{
		if (Tools::getValue('search_query') || !$this->isCached('novadvancedsearch.tpl', $this->getCacheId()))
		{
	...
	public function hookdisplayAdvanceSearch($params)
	{
		...
		if (Tools::getValue('search_query') || !$this->isCached('novadvancedsearch-top.tpl', $key))
		{
			$categoriesTree  = Category::getRootCategory()->recurseLiteCategTree(0);
			$id_category =  Tools::getValue('id_category') ? Tools::getValue('id_category') : 1;
			$this->calculHookCommon($params);
			$this->smarty->assign(array(
